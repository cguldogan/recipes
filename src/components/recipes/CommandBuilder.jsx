...
export function CommandBuilder({ recipe, strategies, taxonomy }) {
  ...
  const [nodeCount, setNodeCount] = useState(() => {
    ...
    // single-node. Same fit check the hardware-change handler runs. (Non-
    // scalable single-box hardware already returned 1 above.)
    const v = recipe.variants?.[variant] || recipe.variants?.default || {};
  ...
  const hwByBrand = useMemo(() => {
    const NVIDIA_ORDER = ["h100", "h200", "b200", "gb200", "b300", "gb300", "rtx_4090_2x", "rtx_5090", "rtx_5090_2x", "rtx_pro_6000", "dgx_station_gb300"];
  ...
  // Non-scalable hardware (single-GPU workstation like DGX Station, or a mixed-
  // card rig pooled via pipeline-parallel) can't add nodes: multi-node is off
  // and oversized variants are disabled. Clamp to a single node whenever the
  // active hardware is non-scalable, however nodeCount got set.
  const hwScalable = isHardwareScalable(hwProfile);
  useEffect(() => {
    if (!hwScalable && nodeCount !== 1) setNodeCount(1);
  }, [hwScalable, nodeCount]);
  ...
  const result = useMemo(
    () => {
      ...
      const pdNodes = activeStrategy === "pd_cluster"
        ...
        : null;
      // taxonomyWithRig, not taxonomy: the combined "All N cards" rig pool is a
      // synthetic profile injected on top of the taxonomy, and synthesis needs
      // to resolve it to emit the pipeline-parallel command.
      return resolveCommand(recipe, variant, activeStrategy, hwId, features, strategies, taxonomyWithRig, advArgs, nodeCount, pdNodes, featureModes, activeKvOffload || null, { count: kvInstances ?? undefined, current: kvInstanceIdx });
    },
    [recipe, variant, activeStrategy, hwId, features, featureModes, advanced, advancedById, strategies, taxonomyWithRig, nodeCount, effPdPrefillNodes, effPdDecodeNodes, pdPrefillRank, pdDecodeRank, effPdPrefillPar, effPdDecodePar, activeKvOffload, kvInstances, kvInstanceIdx]
  );
            ...
            >
              <PillGroup>
                {nodeOptions.map((n) => {
                  ...
                  // clustered (single-chassis workstation such as DGX Station,
                  // or a mixed-card rig pooled via pipeline-parallel).
                      ...
                      title={
                        noMultiNode
                          ? isXpuHardware
                            ...
                            : !hwScalable
                            ? `${hwProfile.display_name || "This hardware"} is a single machine and can't be clustered into multiple nodes.`
